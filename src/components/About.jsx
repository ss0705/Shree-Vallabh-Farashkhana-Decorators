...
import { MapPin, Phone, Award, Calendar, Users, Headphones, Map, Sparkles } from 'lucide-react'
...
const parseStat = (value) => {
  ...
  if (!match) return { target: 0, suffix: '', finalText: String(value), canCount: false }
...
function CountUpStat({ value }) {
  ...
  const { target, suffix, finalText, canCount } = useMemo(() => parseStat(value), [value])
  const [count, setCount] = useState(() => (shouldReduceMotion || !canCount ? target : 0))
  ...
  const displayValue = !canCount || count >= target ? finalText : `${formattedCount}${suffix}`

  return <strong ref={ref} aria-hidden="true">{displayValue}</strong>
}
...
function About() {
  ...
  // Your image URL
  const ownerImageUrl = "https://res.cloudinary.com/deykvluax/image/upload/v1779476139/Gemini_Generated_Image_pzzbx2pzzbx2pzzb_bqlpwo.png"

  // Get icon for each stat based on label
  const getStatIcon = (label, index) => {
    const labelLower = (label || '').toLowerCase()
    if (labelLower.includes('event') || labelLower.includes('decorated')) return Calendar
    if (labelLower.includes('year') || labelLower.includes('experience')) return Award
    if (labelLower.includes('support')) return Headphones
    if (labelLower.includes('service') || labelLower.includes('vadodara')) return Map
    return Users
  }

  return (
    <section className="py-16 md:py-24 bg-gradient-to-br from-amber-50 via-white to-amber-50 overflow-hidden" id="about">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 mb-16">
          ...
          >
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-xl">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <h3 className="text-white text-xl font-bold">{content.ownerName}</h3>
                <p className="text-amber-300 text-sm">Founder & Lead Decorator</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-lg border border-amber-100 space-y-3">
              <div className="flex items-center gap-3 text-gray-700">
                ...
                <div>
                  ...
                  <a href={`tel:+91${content.mobile}`} className="font-semibold hover:text-amber-600 transition">
                    {content.mobile}
                  </a>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-5 content-start">
            {content.stats?.map((stat, index) => {
              ...
              const labelText = isGujarati ? (stat.labelGu || stat.label) : (stat.labelEn || stat.label)
        ...
        >
          <div className="bg-amber-50 rounded-2xl p-5 border border-amber-200">
            <p className="text-amber-800 font-medium text-sm md:text-base">
              ✨ Trusted by over 500+ families in Vadodara for memorable wedding decorations ✨
            </p>
          </div>
        </motion.div>
      </div>
